...
pub struct Add<'info> {
    #[account(
        mut,
        has_one = authority @ ErrorCode::UnauthorizedOperation,
        constraint =
...
pub struct Remove<'info> {
    #[account(mut, has_one = authority @ ErrorCode::UnauthorizedOperation)]
    pub state: Account<'info, ProgramState>,
    pub user_account: Account<'info, UserAccount>,
    pub authority: SystemAccount<'info>,
}
...
#[error]
pub enum ErrorCode {
    #[msg("Capacity exceeded. Try closing the pool and initializing it with a bigger capacity.")]
    CapacityExceeded,
    #[msg("Unathorized authority for operation.")]
